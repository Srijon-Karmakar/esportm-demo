...
export default function SignupNutritionist() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post("http://localhost:5000/nutritionist/signup", {
        name: formData.name,
      ...
      <div className="grid w-full max-w-[850px] overflow-hidden rounded-[28px] bg-white shadow-[0_40px_80px_rgba(40,8,80,0.12)] md:grid-cols-2">
        ...
        <div className="flex flex-col justify-center p-6 sm:p-8 md:p-12">
          ...
          <p className="mt-1 mb-6 text-sm text-slate-500">
            Join Sportbit and help athletes fuel peak performance.
          </p>
          ...
          <form onSubmit={handleSubmit} className="space-y-3.5">
            ...
            <p className="pt-2 text-center text-sm text-slate-600">
              ...
              <button
                ...
                onClick={() => (window.location.href = "/login-nutritionist")}
              >
                Login
              </button>
            </p>
        ...
        <div className="relative hidden md:block">
          ...
          <div className="absolute bottom-10 left-6 right-6 rounded-xl bg-white/50 backdrop-blur-sm p-4 text-black">
            <h3 className="text-lg md:text-2xl font-extrabold leading-tight">
              Fuel performance the smart way.
            </h3>
            <p className="mt-1 text-xs md:text-sm text-black/90">
              Create tailored nutrition plans and track athlete progress on Sportbit.
            </p>
          </div>
        </div>
      </div>

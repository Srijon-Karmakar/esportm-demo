...
//     }
//     try {
//       const response = await axios.post("http://localhost:5000/physio/signup", {
//         name: formData.name,
...
export default function SignupPhysio() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post("http://localhost:5000/physio/register", {
        name: formData.name,
        email: formData.email,
        password: formData.password,
      });
      ...
      const redirectPath = location.state?.from || "/login3"; // ✅ fixed redirect
      navigate(redirectPath);
    } catch (err) {
      alert(err.response?.data?.message || "Signup failed");
    }
      ...
      <div className="grid w-full max-w-[850px] overflow-hidden rounded-[28px] bg-white shadow-[0_40px_80px_rgba(40,8,80,0.12)] md:grid-cols-2">
        ...
        <div className="flex flex-col justify-center p-6 sm:p-8 md:p-12">
          ...
          <form onSubmit={handleSubmit} className="space-y-3.5">
            <input
              ...
              onChange={handleChange}
              required
              className="w-full rounded-lg border border-slate-300 bg-slate-50 px-4 py-2"
            />
            ...
            <p className="pt-2 text-center text-sm text-slate-600">
              ...
              <button
                type="button"
                className="font-medium text-violet-600 underline underline-offset-2 hover:opacity-80"
        ...
        <div className="relative hidden md:block">
          ...
          <div className="absolute bottom-10 left-6 right-6 rounded-xl bg-white/50 backdrop-blur-sm p-4 text-black">
            ...
            <p className="mt-1 text-xs md:text-sm text-black/90">
              Join Sportbit as a physio and help athletes reach peak recovery and
              performance.
            </p>
          </div>
        </div>
